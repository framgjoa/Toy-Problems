...
Tree.prototype.map = function (callback) {
  ...
  var resultTree = new Tree(callback(this.value));
  //print("Children ", this.children);
  if (this.children){
    //print("How many Children ", this.children.length)
    for (var i = 0; i < this.children.length; i++){
      print(i, "th child is ", this.children[i].value);
      print(i, "th child in callback is ", callback( this.children[i].value) );
      resultTree.children[i] = new Tree( callback(this.children[i].value) );
      //resultTree.children.map(callback);
    }
  }else{ //break case at leaves of tree, ie children = [];
    resultTree.value.map(callback);
  }
...
testTree.map(function(value){ value++}) ;
