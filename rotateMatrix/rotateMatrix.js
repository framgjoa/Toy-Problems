...
var rotateMatrix = function(matrix, rotation) {
  ...
  //print("Matrix row, col: ", rowCount, colCount);
  var results = [];
  var tempRow = [];

  for(var i = rowCount; i>0; i--){ //starting on last row, left hand side
    for(var j = 0; j<colCount; j++){  //think rotation would be a ternary in here
      if(tempRow.length === colCount){  //this should address rect matrixes
        results.push(tempRow);
        tempRow=[];                   //reset to empty
        //print("Results: ", results, "at indicies ", i, " ", j);
      }
      else{
        tempRow.push(matrix[i][j]);
        //print("tempRow: ", tempRow, "at indicies ", i, " ", j);
      }
